...
metric_groups_impl& metric_groups_impl::add_metric(group_name_type name, const metric_definition& md, int handle)  {
    ...
    get_local_impl(handle)->add_registration(id, md._impl->type, md._impl->f, md._impl->d, md._impl->enabled, md._impl->aggregate_labels);
...
shared_ptr<impl> get_local_impl(int handle) {
    ...
    if (!impls.contains(handle)) {
        auto ptr = ::seastar::make_shared<impl>();
        impls[handle] = ptr;
    }
